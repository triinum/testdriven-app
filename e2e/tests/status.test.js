...
test(`should display user info if a user is logged in`, async (t) => {
  ...
  await t
    ...
    .expect(Selector('li > strong').withText('ID:').exists).ok()
    .expect(Selector('li > strong').withText('Email:').exists).ok()
    .expect(Selector('li span.value').withText(email).exists).ok()
    .expect(Selector('li > strong').withText('Username:').exists).ok()
    .expect(Selector('li span.value').withText(username).exists).ok()
